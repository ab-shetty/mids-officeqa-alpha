"""A2A executor that plugs the OfficeQAAgent into the agentbeats runtime.

Shape mirrors officeqa-agentbeats/participant/src/executor.py so we stay
drop-in compatible with the green judge.
"""
...
class Executor(AgentExecutor):
    """One-shot executor: each question is a fresh A2A conversation."""

    def __init__(self, cache_dir: Path | None = None) -> None:
        self._cache_dir = Path(cache_dir or os.environ.get("CORPUS_CACHE_DIR", "/data/corpus"))
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
            ...
            TaskStatusUpdateEvent(
                ...
                status=TaskStatus(
                    ...
                    message=Message(
                        messageId=uuid4().hex,
                        role="agent",
                        parts=[Part(root=TextPart(kind="text", text="Loading corpus + retrieving…"))],
                    ),
                ),
                final=False,
            )
        ...
        try:
            ...
            response = (
                f"<REASONING>Agent error: {e}</REASONING>\n"
                f"<FINAL_ANSWER>0</FINAL_ANSWER>"
            )
